...
export const OptimizedImage = ({ src, alt, width = 400, height = 300, quality = 80, loading = 'lazy', className = '', placeholder = 'color', forceContainerStyle = false, ...props }) => {
    ...
    const containerStyle = isTauri || forceContainerStyle
        ? {
    ...
    useEffect(() => {
        ...
        if (isTauri) {
            ...
            if (src.startsWith('/') ||
                src.startsWith('./') ||
                src.startsWith('../')) {
                optimizedSrc = `https://asset.localhost${src.startsWith('/') ? src : `/${src}`}`;
    ...
    if (error) {
        ...
        `, style: containerStyle, children: _jsxs("div", { className: "text-center", children: [_jsx("svg", { className: "w-12 h-12 text-gray-400 mx-auto", fill: "none", stroke: "currentColor", viewBox: "0 0 24 24", children: _jsx("path", { strokeLinecap: "round", strokeLinejoin: "round", strokeWidth: 1, d: "M4 16l4.586-4.586a2 2 0 012.828 0L16 16m-2-2l1.586-1.586a2 2 0 012.828 0L20 14m-6-6h.01M6 20h12a2 2 0 002-2V6a2 2 0-2-2H6a2 2 0-2 2v12a2 2 0-2 2z" }) }), _jsx("p", { className: "text-sm text-gray-500 mt-2", children: "Failed to load image" })] }) }));
    }
    ...
    if (!imageSrc) {
        return (_jsx("div", { className: `
          ...
          rounded-lg flex items-center justify-center ${className}
        `, style: containerStyle, children: _jsxs("div", { className: "animate-pulse text-center", children: [_jsx("div", { className: "w-12 h-12 bg-gray-300 rounded-full mx-auto mb-2" }), _jsx("p", { className: "text-sm text-gray-400", children: "Loading..." })] }) }));
    }
    return (_jsxs("div", { className: `
        relative w-full overflow-hidden rounded-lg ${className}
      `, style: containerStyle, children: [!loaded && (_jsx("div", { className: "absolute inset-0 animate-pulse bg-gradient-to-r from-blue-50 via-white to-indigo-100 z-10" })), _jsx("img", { ref: imgRef, src: imageSrc, alt: alt, loading: loading, className: `
          w-full h-full object-cover transition-opacity duration-300
          ${loaded ? 'opacity-100' : 'opacity-0'} relative z-20
        `, onLoad: handleLoad, onError: handleError, ...props })] }));
};
